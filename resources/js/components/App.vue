    ...
    <div class="row">
      <div class="col-lg-3">
        <h1 class="my-4">
          <input
            type="text"
            v-model="$root.$data.search$"
            placeholder="Search.."
            class="form-control"
          />
        </h1>
        <div class="my-4">
          <div class="checkbox" v-for="category in $root.$data.categories" :key="category">
            <label>
              <input
                type="checkbox"
                name="category"
                :value="category"
                v-model="$root.$data.selectedCategory"
              />
              {{category}}
            </label>
          </div>
        ...
        <div class="list-group">
          <a
            v-for="(location, index) in $root.fuzzySearch"
          ...
          <div v-if="cardBody.text" class="card-body">
            <div class="row">
              <div class="col-md-7">
                <a href="#">
                  <img
                    id="card-card-image-size"
                    class="img-fluid rounded mb-3 mb-md-0"
                    :src="'/images/' + cardBody.image + '.jpg'"
                    alt
                  />
                </a>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      marker: L.latLng(47.41322, -1.219482), //marker
      //                v-map props
      zoom: 13,
